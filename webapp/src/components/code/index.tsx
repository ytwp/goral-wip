...
import { buildAllSuggestionsToEditor, convertCodeEditTheme } from './function';
import { handleInitEditorAndLanguageOnBeforeMount } from './function';
import { SuggestionInfo } from '@/types/code-editor';
...
const CodeEdit = (props: CodeEditFormProps) => {
  ...
  const buildAllSuggestions = (model: ITextModel, position: monaco.Position) => {
    return buildAllSuggestionsToEditor(model, position, []);
  ...
  function reloadCompilation(monacoIns: Monaco, segmentedWords: string[]) {
    ...
    provider = monacoIns.languages.registerCompletionItemProvider(language, {
      provideCompletionItems: (
        ...
        const suggestions: Promise<languages.CompletionList> = allSuggestions.then((res: { suggestions: any; }) => {
          return {
            suggestions: [
              ...(res?.suggestions ?? []),
